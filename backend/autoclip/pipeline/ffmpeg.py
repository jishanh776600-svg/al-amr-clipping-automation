...
def ffmpeg_path() -> str:
    ...
    if not exe:
        import sys
        env_exe = Path(sys.executable).parent / ("ffmpeg.exe" if sys.platform == "win32" else "ffmpeg")
        if env_exe.exists():
            return str(env_exe)
        raise FFmpegError(
...
def ffprobe_path() -> str:
    ...
    if not exe:
        import sys
        env_exe = Path(sys.executable).parent / ("ffprobe.exe" if sys.platform == "win32" else "ffprobe")
        if env_exe.exists():
            return str(env_exe)
        raise FFmpegError(
