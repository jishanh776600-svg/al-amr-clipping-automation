...
def _clip_out(clip) -> ClipOut:
    ...
    evaluation = store.get_campaign_evaluation(clip.id)
    return ClipOut.of(clip, edit=edit, exports=exports, evaluation=evaluation)
...
async def list_all_clips(limit: int = 50, status: str | None = None) -> list[ClipOut]:
    """List recent clips across all jobs for the Clip Gallery."""
    clips = await asyncio.to_thread(store.list_recent_clips, limit=limit, status=status)
    return [await asyncio.to_thread(_clip_out, clip) for clip in clips]
