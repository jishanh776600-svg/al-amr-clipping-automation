...
def clip(
    ...
    campaign_file: Path | None = typer.Option(
        None, "--campaign", "-c", help="Path to a JSON file containing a campaign brief."
    ),
) -> None:
    ...
    if campaign_file:
        ...
        if not campaign_file.is_file():
            console.print(f"[red]Campaign file not found:[/red] {campaign_file}")
            raise typer.Exit(1)
        job_settings["campaign"] = json.loads(campaign_file.read_text(encoding="utf-8"))
